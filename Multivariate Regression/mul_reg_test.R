...
formula(close_pred)
drop1(close_pred, test='F')
# range_norm has the lowest AIC (-2296.1) and it is not significant, hence it is dropped.
close_pred <- lm(close_norm~open_norm+low_norm+high_norm, data=train)
formula(close_pred)
drop1(close_pred, test='F')
# No further predictors can be dropped, since both are significant.
summary(close_pred)
close_pred$residuals
plot(close_pred$residuals, xlab = "Time points", ylab = "Residual values", lwd =2, xlim = c(0, 300))
